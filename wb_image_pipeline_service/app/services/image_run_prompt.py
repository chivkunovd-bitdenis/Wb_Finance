...
_DEFAULT_TEMPLATE = """Ты готовишь вход для JSON-планировщика, который затем выдаст 4 отдельных промпта под генерацию главных фото карточки Wildberries по референсу.

Цель: 4 заметно разных главных кадра одного товара: высокое качество, реализм, современный e-commerce / editorial, без «четырёх копий одного ракурса».

Жёсткий инвариант по товару:
- генерировать только предмет с референса;
- не менять категорию, крой, тип изделия, ключевые детали, материал и цвет;
- если текст продавца уточняет товар — это приоритетнее общих стилистических слов шаблона.

Разнообразие (обязательно отрази в пожеланиях продавца или в явных требованиях к планировщику):
- четыре разных схемы кадра из набора: поясной / до колена / полный рост / крупный план фактуры (ворот, пуговицы, ткань);
- минимум два разных ракурса камеры: чуть снизу, чуть сверху, сбоку ~45°, со спины с поворотом, динамика шага;
- минимум две разные локации или световых схемы (студия нейтральная / улица / интерьер / мягкий контровой);
- позы не копируй: не более одного «стоит анфас руки вдоль тела»; остальное — жест, шаг, сидя, поправляет воротник и т.п.;
- модели могут отличаться внешностью (если продавец не требует обратного), но товар на всех кадрах тот же.
