...
- какие визуальные направления лучше всего могут раскрыть именно этот товар.
...
Этап 4 main — это выбор визуального направления будущей фотосессии.
Сделай 4 разных fashion/e-commerce концепта одного товара, чтобы пользователь выбирал не обрезку кадра, а стиль всей будущей карточки.
Товар одинаковый, но модель, образ, фон, локация, настроение и styling должны быть разными.
Белый, нейтральный или студийный фон допустим максимум в одном варианте из 4.
Остальные варианты должны быть lifestyle/editorial сценами с разным характером.

Обязательные визуальные направления:
1. Чистый premium e-commerce: аккуратный светлый кадр, можно студийный или минималистичный.
2. Old money / quiet luxury: веранда, кафе, городская улица, спокойный дорогой образ.
3. Casual weekend / лёгкий лайфстайл: расслабленный образ, кеды/шорты/джинсы или другой уместный casual styling.
4. Bold fashion / editorial: более дерзкая fashion-подача, выразительная модель, очки/юбка/брюки или другой уместный styling, контрастная городская или архитектурная сцена.

Если товар не одежда, адаптируй эти 4 направления под предметную съёмку: premium catalog, quiet luxury interior, casual usage/lifestyle, bold editorial still life.
...
- каждый промпт должен явно описывать свой визуальный стиль, локацию, образ модели/сетап, настроение, фон, свет, позу и композицию;
- 4 промпта должны быть разными по visual style, модели/образу, локации, styling-настроению и коммерческой подаче;
- не делай 4 технические вариации одного и того же светлого кадра;
- не делай 4 одинаковых средних фронтальных кадра;
- если товар на модели, между 4 вариантами можно менять модель, фон, позу, свет, нижнюю одежду, обувь, аксессуары и styling, но нельзя менять сам товар;
- если товар предметный, между 4 вариантами можно менять сетап, фон, композицию и свет, но нельзя менять сам товар;
- аксессуары и окружение допустимы только как часть образа, если они не перекрывают и не меняют товар;
- не добавляй текст, логотипы, лишние надписи и несуществующие детали;
