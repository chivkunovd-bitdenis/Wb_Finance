...
- все 4 промпта должны описывать тот же самый товар, который есть на референсе и уточнён текстом продавца;
- нельзя менять категорию товара: поло не превращается в обувь, брюки — в юбку, платье — в сумку;
- можно менять модель, позу, фон, локацию, свет, композицию и визуальный стиль;
- каждый промпт должен явно требовать сохранить форму, цвет, материал, конструктивные детали и пропорции товара с референса;
- промпты должны быть разными по подаче, чтобы пользователь мог выбрать стиль главного фото для WB.
