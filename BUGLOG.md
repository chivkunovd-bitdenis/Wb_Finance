...
Дата: 2026-05-16
Статус: fixed
Автоматизация: нет (ручной репорт с прода)

## Бизнес-описание
На вкладке «Себестоимость» интерфейс периодически мигал (полноэкранный лоадер каждые ~5 с), мешая вносить себестоимость.

## Процесс / сценарий
1) Пользователь открывает `/costs`.
2) При фоновой синхронизации `Layout` каждые 5 с увеличивает `refreshTrigger`.
3) Ожидаемо: таблица остаётся на экране.
4) Фактически: `Costs.jsx` ставил `loading=true` и показывал только спиннер.

## Техническое описание
`Layout.jsx` polling → `refreshTrigger++`. `Costs.jsx` при каждом refresh скрывал таблицу лоадером. На `Articles.jsx` уже был `showFullLoader = loading && data.length === 0`.

## Root cause (почему произошло)
- Недоучтён фоновый `refreshTrigger` при уже загруженных данных.

## Исправление (что сделали)
`showFullLoader` только при пустых данных; при refresh не перезаписывать отредактированные поля себестоимости.

## Профилактика (как не повторить)
Тот же паттерн `showFullLoader` для экранов с `refreshTrigger`.

## Проверка
- `npm run lint`, `npm run build`
- На проде: «Себестоимость» при активном sync — без мигания

Затронутые файлы: frontend/src/screens/Costs.jsx, frontend/dist/*
---
ID: BUG-12
Дата: 2026-04-30
Статус: fixed
