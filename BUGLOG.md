...
Если сессия уже запущена, `POST /start` теперь всегда пытается выполнить `page.goto("https://seller.wildberries.ru/")`. Если навигация не удалась — сессия пересоздаётся.
...
Затронутые файлы: `backend/app/wb_auth_manager.py`, `BUGLOG.md`
---
