...
## Бизнес-описание
У `vitalik-hors@mail.ru`, `sherin-ivan@ya.ru`, `m.soloveva333@mail.ru`, `natalya.khv@mail.ru` данные остановились на 2026-09-17: ни финансов, ни воронки за 18–20 сентября. Пользователь видит «оцифровка не работает 4 дня».

## Процесс / сценарий
1) Утром 2026-09-19 (07:14 UTC) WB на `statistics-api/.../reportDetailByPeriod` ответил 429 с `X-RateLimit-Reset=314451` (≈3,6 суток) для ключа Виталика; через 4 минуты — Шерину 296223. Значение убывает ровно по часам → это фиксированная дата разблокировки (Виталик ≈ 2026-09-22 22:35 UTC), повторные 429 её не сдвигают.
2) Оркестратор берёт окно из заголовка, режет до `WB_MAX_RETRY_AFTER_SEC` (12 ч) и уходит в **глобальный** cooldown на пользователя.
3) Воронка (`funnel_tail`, seller-analytics-api, свои лимиты ~45 с) стоит в той же очереди и не грузится, хотя ей ничего не мешает.
4) Раз в 12 часов повтор → снова 429 → ещё 12 часов. Ключ владельца (`denischivkunov@icloud.com`) с идентичными claims в JWT (`acc=3, ent=1, s=…`) синкается нормально — разница только в частоте обращений к этому эндпоинту (владелец — раз в 1–2 недели большим диапазоном, остальные — каждый день).

## Техническое описание
`wb_orchestrator_tick`: любой HTTPError 429/5xx → `_orch_set_cooldown` на весь state. Понятия «какой WB API заблокирован» не было. В документации WB (dev.wildberries.ru, проверено в браузере 21.09.2026) метод reportDetailByPeriod поиском не находится; для новых `finance-api/.../sales-reports/detailed` лимиты зависят от типа токена (Базовый: 2 запроса/24 ч).

## Root cause (почему произошло)
- WB ужесточил лимит на устаревший эндпоинт финансового отчёта (динамика reset_sec: 475 с 01.09 → 3826 с 14.09 → 314451 с 19.09).
- Наш cooldown — глобальный на пользователя, а не на операцию; блок финансов замораживал воронку.
...
Дата: 2026-09-19
Статус: fixed
Автоматизация: нет (репорт пользователя `vitalik-hors@mail.ru`; воспроизведено руками на мок-бэкенде с pending funnel_tail)

## Бизнес-описание
Пока висит плашка «Догружаем финансы и воронку по пропущенным дням», экран «Себестоимость» мигает каждые ~5 секунд и сбрасывает всё, что пользователь ввёл: налоговую ставку нельзя сохранить (значение возвращается к старому раньше, чем нажмёшь «Сохранить»), правки себестоимости тоже пропадают. На «Дашборде» так же затираются введённые планы в план-факте.

## Процесс / сценарий
1) У пользователя оркестратор WB в `cooldown` после 429 (у Vitalik — на 12 часов), intent `funnel_tail` висит как `pending`.
2) `/dashboard/state` отдаёт `funnel_tail_sync.pending=true, status='cooldown'` → фронт считает синхронизацию активной.
3) Layout каждые 5 с опрашивает `/dashboard/state` **и** инкрементит `refreshTrigger`.
4) Costs на каждый `refreshTrigger` ставит `loading=true` → вместо карточки рендерится лоадер → `TaxRateBlock` размонтируется и при повторном монтировании заново читает ставку с сервера. Поля себестоимости перезаписываются `setCosts(c)` из ответа.

## Техническое описание
`frontend/src/Layout.jsx`: эффект опроса при `financeMissingActive || funnelTailActive` вызывал `setRefreshTrigger` на каждом тике, независимо от того, продвинулся ли оркестратор. `frontend/src/screens/Costs.jsx`: фоновая перечитка списка вела себя как первичная загрузка (полноэкранный лоадер + полная замена введённых значений).

## Root cause (почему произошло)
- «Опросить состояние» и «перечитать данные экранов» были склеены в одно действие; при длинном cooldown от WB это превращается в бесконечный сброс форм.
- Экран с формами не отличал первичную загрузку от фоновой перечитки.

## Исправление (что сделали)
- `storeDataGuard.syncProgressSignature(state)` — отпечаток прогресса (max_date, статус/диапазон finance_missing_sync, pending/last_step funnel_tail). Layout бампит `refreshTrigger` только при смене отпечатка; сам опрос state каждые 5 с остался (плашка обновляется).
- Costs: лоадер не включается заново при перечитке (карточка и блок налоговой ставки остаются смонтированными); nm_id, которые пользователь правит, помечаются как dirty и не перетираются ответом сервера; dirty сбрасывается после успешного сохранения.

## Профилактика (как не повторить)
Любой polling в Layout не должен дёргать `refreshTrigger` «на всякий случай» — только по факту изменения данных. Экраны с формами: фоновая перечитка не должна размонтировать форму и затирать локальные правки.

## Проверка
- Руками в браузере (vite dev + мок API с `funnel_tail_sync.pending=true, status='cooldown'`): до фикса введённое 7.5 через 7 с возвращалось к 6; после фикса 7.5 и правка себестоимости 555 живут >13 с (3 опроса state, `/dashboard/articles` не перечитывается). После «завершения» синхронизации в моке — плашка гаснет, articles перечитываются один раз, введённая ставка сохраняется в поле.
- `npx eslint src/Layout.jsx src/screens/Costs.jsx src/storeDataGuard.js` — чисто; `npm run build`.

Затронутые файлы: `frontend/src/Layout.jsx`, `frontend/src/screens/Costs.jsx`, `frontend/src/storeDataGuard.js`, `frontend/vite.config.js` (прокси `/billing` для dev), `frontend/dist/*`, `BUGLOG.md`
...
Дата: 2026-06-02
Статус: fixed
Автоматизация: да (pytest: wake stale `running` в оркестраторе; wake stale `running` missing-tail при входе)

## Бизнес-описание
Пользователь открывает дашборд и видит нули/отсутствие данных за последние дни, хотя раньше данные были. Обновление страницы и повторные входы не помогают: витрина “застывает” на старой дате.

## Процесс / сценарий
1) Пользователь открывает дашборд.
2) Система должна догрузить последние дни в фоне и через некоторое время показать актуальные значения.
3) Фактически: догрузка считалась “уже запущенной”, но прогресс не менялся, и повторные входы не пробуждали процесс.

## Техническое описание
Точечная догрузка последних дней использует служебную запись прогресса (“в процессе/готово/ошибка”), чтобы не плодить одинаковые фоновые задачи на каждый refresh. Если эта запись оставалась в статусе “в процессе” после смерти фонового шага (рестарт/kill/ошибка), последующие входы переставали пробовать догрузку.
Параллельно, если оркестратор оставался в статусе “в процессе” без живого тика, повторные “пинки” не планировали новый тик.

## Root cause (почему произошло)
- Недоучтён кейс: фоновой шаг мог умереть после выставления статуса “в процессе”, но до выставления “готово/ошибка”.
- Дедуп-логика на входе в дашборд считала “в процессе” абсолютным стоп-сигналом и не умела “оживлять” stale состояние.
- “Пинок” оркестратора не планировал тик, если persisted статус оставался “в процессе”, даже если тик уже не крутился.

## Исправление (что сделали)
- Оркестратор: повторный “пинок” теперь пробуждает тик, если persisted статус “в процессе” выглядит stale по времени обновления.
- Дашборд: если прогресс точечной догрузки “в процессе”/“в очереди” долго не менялся, выполняем идемпотентный повторный “пинок” (без шторма запросов).

## Профилактика (как не повторить)
Добавлены регрессионные тесты на “stale running” для оркестратора и на авто‑пробуждение точечной догрузки при входе на дашборд.

## Проверка
- Команды: `ruff check .`, `mypy .`, `pytest -q backend/tests/test_wb_orchestrator_intents_merge.py backend/tests/test_finance_holes_process.py`
- Сценарии: на проде вручную догнали витрину у пользователей, у которых дата остановилась на 2026-05-29; после фикса повторный “пинок” должен оживлять зависшие состояния без ручного сброса.

Затронутые файлы: `backend/celery_app/tasks.py`, `backend/app/routers/dashboard.py`, `backend/tests/test_wb_orchestrator_intents_merge.py`, `backend/tests/test_finance_holes_process.py`, `BUGLOG.md`, `TASKLOG.md`
