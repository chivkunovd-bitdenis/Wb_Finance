...
1. Монолит сохраняет черновик (формы + пути референсов).
...
- **Шаги (логика):** (1) Референсы + текст + габариты + цена + артикул + название + бренд + таблица размеров; (2) подтверждение; (3) **Создать** → закрытие.
...
- `vendor_code`, `title`, `brand`, `description_user` (исходный текст)
