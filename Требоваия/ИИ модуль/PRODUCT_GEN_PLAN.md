...
> **Актуализация (2026-05-13):** UX и бэклог разделены на поток **IMAGE** (референсы + пользовательский текст + шаблон промпта в воркере → run) и поток **PRODUCT/WB** (поля карточки и публикация — после фото). Разделы **§2–§4** ниже частично отражают старый сценарий «заполнить форму → Создать → старт»; привести их в соответствие с v2 — задача **PG-A.0** в `PRODUCT_GEN_TASKS.md`.
