...
отдельная переменная AI_CFO_MODEL (по умолчанию "gpt-5.4": на живых данных 30 дней — 40 с, проходит весь чеклист; gpt-4.1 заметно поверхностнее), т.к. это тяжёлый разовый анализ,
...
_AI_CFO_MODEL = os.getenv("AI_CFO_MODEL") or "gpt-5.4"
...
4. Структура отчета:
   ...
   - СТРАТЕГИЧЕСКИЙ ВЫВОД (2-3 предложения: куда двигать портфель).
