...
def run_daily_analytics(
    *,
    ...
    Inputs:
    - **WB «Сравнение карточек»** (импортированный отчёт): строки «Показы», «Конверсия в корзину, %», «Конверсия в заказ, %»
      (опционально «CTR»). Показы — абсолют; по конкурентам в файле — **среднее** показов. Конверсии/CTR в ячейках как в WB
      (часто без «%») — процентные пункты; по конкурентам — **медиана** (нули не берём).
