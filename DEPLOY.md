...
> Важно: **конкретный путь проекта зависит от сервера**.
>
> - Для вашего Zurich VPS `194.87.96.144` путь проекта: **`/root/wb-finance`** (см. `SERVER_ZURICH.md` и `SERVER_RUNBOOK.md`)
> - В примерах ниже используется “универсальный” путь **`/opt/wb-finance`**. Если вы деплоите на Zurich VPS — **заменяйте `/opt/wb-finance` на `/root/wb-finance`**.
...
> Для Zurich VPS используйте “один выстрел” из `SERVER_RUNBOOK.md`.
